...
class ExoplayerView : AppCompatActivity(), Player.Listener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fastforward = playerView.findViewById<ImageView>(R.id.fastforward)
        ...
        serverInfo.text = model.watchSources!!.names.getOrNull(media.selected!!.source)
    ...
    private fun initPlayer() {
        ...
        var subConfig: MediaItem.SubtitleConfiguration? = null
        if (subtitle != null) {
            subConfig = MediaItem.SubtitleConfiguration.Builder(Uri.parse(subtitle!!.file.url))
        ...
        if (video?.format == VideoType.CONTAINER || (loadData<Int>("settings_download_manager")
            ...
            but.setOnClickListener { download(this, episode, animeTitle.text.toString()) }
        } else {
        ...
        val httpClient = okHttpClient.newBuilder()
            ...
            .addInterceptor { chain ->
                ...
                defaultHeaders.forEach { builder.addHeader(it.key, it.value) }
                video?.file?.headers?.forEach { builder.addHeader(it.key, it.value) }
        ...
        if (subConfig != null) {
            videoMediaItemBuilder.setSubtitleConfigurations(listOf(subConfig))
        }
        ...
        val audioSources = audioMediaItems.map { item ->
            if (item.localConfiguration?.uri.toString().contains(".m3u8", ignoreCase = true)) {
                ...
                DefaultMediaSourceFactory(cacheFactory).createMediaSource(item)
            }
        }.toTypedArray()
