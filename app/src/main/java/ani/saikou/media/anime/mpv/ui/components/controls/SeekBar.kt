    ...
    val seekerSegments = remember(skipStamps, durationMs) {
        ...
        if (skipStamps.isNullOrEmpty() || durationMs <= 0L) {
            ...
            sortedStamps.forEach { stamp ->
                ...
                val endMs = stamp.endTimeMs.toFloat()
                if (startMs < durationMs) {
                    list.add(Segment(name = stamp.type, start = startMs, color = unifiedTrackColor))
                    if (endMs < durationMs) {
                        list.add(Segment(name = "${stamp.type}_End", start = endMs, color = unifiedTrackColor))
                    }
                }
            }
